...
@section('title', 'Yeni Ecrimisil Kaydı')
...
<style>
    ...
    .ec-mt-btn {
        ...
        background: rgba(255,255,255,.06); color: #dfe3ea; border: 1px solid rgba(255,255,255,.08);
    ...
    .ec-mt-durum {
        margin-left: auto; padding: 5px 12px; background: rgba(255,255,255,.04);
        border-radius: 6px; font-size: .74rem; color: #9aa3b2; display: inline-flex; gap: 8px;
    }
    ...
    /* Poligon vertex stilleri — taşınmaz ekleme ile aynı */
    .ec-vertex-wrap { background: none !important; border: none !important; }
    .ec-vertex {
        width: 12px; height: 12px; background: #fff; border: 2px solid #f43f5e;
        border-radius: 50%; box-shadow: 0 1px 3px rgba(0,0,0,.4);
    }
    .ec-vertex-first {
        width: 16px; height: 16px; background: #f43f5e; border-color: #fff;
        box-shadow: 0 0 0 2px #f43f5e, 0 1px 4px rgba(0,0,0,.4);
        cursor: pointer; animation: ec-pulse 1.6s ease-in-out infinite;
    }
    @keyframes ec-pulse { 0%,100%{transform:scale(1);} 50%{transform:scale(1.18);} }
    .ec-vertex-edit {
        width: 14px; height: 14px; background: #0e1726; border-color: #f43f5e;
        cursor: move; box-shadow: 0 1px 4px rgba(0,0,0,.5);
    }
    .ec-vertex-edge {
        width: 10px; height: 10px; background: rgba(244,63,94,.85);
        border: 1.5px solid #fff; border-radius: 50%; cursor: cell; opacity: .6;
    ...
    .ec-ipucu {
        ...
        font-size: .78rem; min-height: 36px; display: flex; align-items: center;
        transition: background .2s ease;
    }
    .ec-ipucu.is-aktif { background: rgba(245,158,11,.12); color: #fcd34d; border-left-color: #f59e0b; }

    /* Bağlanan taşınmaz bilgi kutusu */
    ...
    /* Mevcut ecrimisil poligonları — harita üstünde tooltip */
    .ec-mvt-tooltip {
        background: rgba(15,23,42,.95) !important; color: #fff !important; border: none !important;
        padding: 6px 10px !important; border-radius: 6px !important; font-size: .78rem !important;
...
<section class="page-hero">
    <div class="page-hero-text">
        <h2>Yeni İşgal Kaydı</h2>
        <small>İşgalci bilgilerini girin; tutanak ve resimler kaydettikten sonra eklenir.</small>
    </div>
    <div class="page-hero-actions">
        <a href="{{ route('panel.ecrimisil.index') }}" class="btn-cancel">Vazgeç</a>
    </div>
</section>
...
<form method="POST" action="{{ route('panel.ecrimisil.kaydet') }}" id="ec-form">
    @csrf

    {{-- ==== HARİTA — İşgal alanını çiz ==== --}}
    <div class="ec-harita-wrap">
        <div class="ec-harita-baslik">
            <h3>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s-7-7-7-13a7 7 0 0114 0c0 6-7 13-7 13z"/><circle cx="12" cy="9" r="2.6"/></svg>
                İşgal Alanı — Harita
            </h3>
            <small>Poligon çiz, sürükleyerek düzenle veya KML yükle</small>
        </div>

        <div class="ec-harita-toolbar">
            <button type="button" class="ec-mt-btn" id="ec-btn-poligon">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2L2 8l3 12h14l3-12z"/></svg>
                Poligon Çiz
            </button>
            <button type="button" class="ec-mt-btn is-tehlike" id="ec-btn-sil" hidden>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2M6 6l1 14a2 2 0 002 2h6a2 2 0 002-2l1-14"/></svg>
                Sil
            </button>
            <button type="button" class="ec-mt-btn" id="ec-btn-konum">
            ...
            <label class="ec-mt-btn" for="ec-kml-input">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 4v12m0 0l-4-4m4 4l4-4M4 20h16"/></svg>
                KML Yükle
            </label>
            <input type="file" id="ec-kml-input" accept=".kml" style="display:none;">
            <button type="button" class="ec-mt-btn" id="ec-btn-tam-ekran">
        ...
        <div class="ec-ipucu" id="ec-ipucu">Poligon Çiz'e basıp haritada tıklayarak nokta ekleyin, ilk noktaya tıklayarak kapatın. Poligon üzerine tıklayınca düzenleme aktifleşir; vertex sürükleyin, ara noktaları yeni vertex için sürükleyin.</div>
    ...
    <section class="form-section">
        <div class="form-section-head">
            ...
            <div class="form-section-head-text">
                <h3>Şirket / Kişi Bilgileri</h3>
                <small>Gerçek kişi veya tüzel kişi (şirket) bilgileri.</small>
            </div>
        </div>
        <div class="form-row form-row-2">
            <div class="form-field">
                <label for="ad_soyad_unvan">Ad Soyad / Şirket Ünvanı *</label>
                <input type="text" name="ad_soyad_unvan" id="ad_soyad_unvan" class="form-input" value="{{ old('ad_soyad_unvan') }}" required maxlength="200">
            </div>
            <div class="form-field">
                <label for="tc_vergi_no">TC Kimlik / Vergi No</label>
                <input type="text" name="tc_vergi_no" id="tc_vergi_no" class="form-input" value="{{ old('tc_vergi_no') }}" maxlength="20" placeholder="TC (11) veya Vergi No (10)">
            </div>
        </div>
        <div class="form-row form-row-2">
            <div class="form-field">
                <label for="nitelik">İşgal Niteliği</label>
                <input type="text" name="nitelik" id="nitelik" class="form-input" value="{{ old('nitelik') }}" maxlength="150" placeholder="Bakkal, büfe, satış ofisi, depo vb.">
            </div>
            <div class="form-field">
                <label for="cadde_sokak">Cadde / Sokak</label>
                <input type="text" name="cadde_sokak" id="cadde_sokak" class="form-input" value="{{ old('cadde_sokak') }}" maxlength="200">
            </div>
        </div>
        <div class="form-field">
            <label for="adres">Açık Adres</label>
            <input type="text" name="adres" id="adres" class="form-input" value="{{ old('adres') }}" maxlength="500">
        </div>
    </section>
    ...
    <section class="form-section">
        <div class="form-section-head">
            ...
            <div class="form-section-head-text">
                <h3>Taşınmaz & Sorumlu</h3>
                <small>İşgal edilen taşınmaz ve takipten sorumlu kullanıcı.</small>
            </div>
        </div>
        <div class="form-row form-row-2">
            <div class="form-field">
                <label>Bağlanan Taşınmaz</label>
                <input type="hidden" name="tasinmaz_id" id="tasinmaz_id" value="{{ old('tasinmaz_id', $tasinmaz?->id) }}">
                <div class="ec-tsn-bilgi" id="ec-tsn-bilgi">
                    @if ($tasinmaz)
                        <div class="ec-tsn-durum is-bulundu">
                            ...
                            <div>
                                <strong>Ada {{ $tasinmaz->ada }} / Parsel {{ $tasinmaz->parsel }}</strong>
                                <small>{{ $tasinmaz->il?->ad }} · {{ $tasinmaz->ilce?->ad }} · {{ $tasinmaz->mahalle?->ad }}</small>
                            </div>
                        </div>
                    @else
            ...
            <div class="form-field">
                <label for="kullanici_id">Sorumlu Kullanıcı</label>
                <select name="kullanici_id" id="kullanici_id" class="form-select">
                    <option value="">— Atanmadı —</option>
                    @foreach ($kullanicilar as $k)
                        <option value="{{ $k->id }}" @selected(old('kullanici_id') == $k->id)>{{ $k->ad }} {{ $k->soyad }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="form-field">
            ...
            <textarea name="aciklama" id="aciklama" class="form-input" rows="3" maxlength="5000">{{ old('aciklama') }}</textarea>
        </div>
    </section>

    <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:16px;">
        <a href="{{ route('panel.ecrimisil.index') }}" class="btn-cancel">Vazgeç</a>
        <button type="submit" class="btn-submit">Kaydet ve Devam Et</button>
    </div>
</form>
@endsection
...
<script>
/* ------------------------------------------------------------------
 * ECRIMISIL — Özel poligon çizim motoru (leaflet.draw kullanmaz)
 * Taşınmaz ekleme sayfasındaki motorun ecrimisil için sadeleştirilmiş sürümü.
 * ------------------------------------------------------------------ */
(function () {
    'use strict';
    ...
    const eskiKoordinat = @json(old('koordinat', isset($kayit) && $kayit->koordinat ? json_encode($kayit->koordinat) : null));
    ...
    const HARIC_ID = @json(isset($kayit) ? $kayit->id : null);
    const CSRF = @json(csrf_token());

    // Referans taşınmaz koordinatı
    @php
        $refKoord = null;
        if (isset($tasinmaz) && $tasinmaz) {
            $k = $tasinmaz->relationLoaded('koordinat') ? $tasinmaz->koordinat
                : \App\Models\TasinmazKoordinat::where('tasinmaz_id', $tasinmaz->id)->first();
            if (! empty($k)) {
                $refKoord = ['koordinat' => $k->koordinat, 'lat' => $k->lat, 'lng' => $k->lng];
            }
        }
    @endphp
    const refKoordinat = @json($refKoord);

    // ---------- Harita (zoom kontrolü YOK) ----------
    const harita = L.map('ec-harita', {
        zoomControl: false,
    ...
    if (refKoordinat) {
        try {
            ...
            let ref = null;
            if (g && g.type === 'Polygon') {
                ref = L.geoJSON({ type: 'Feature', geometry: g }, {
                    style: { color: '#facc15', fillColor: '#facc15', fillOpacity: .1, weight: 1.8, dashArray: '5 5' },
                    interactive: false,
                }).addTo(harita);
                harita.fitBounds(ref.getBounds(), { padding: [30, 30] });
            } else if (Array.isArray(g) && Array.isArray(g[0])) {
                ref = L.polygon(g.map(p => [p[1], p[0]]), { color: '#facc15', fillColor: '#facc15', fillOpacity: .1, weight: 1.8, dashArray: '5 5', interactive: false }).addTo(harita);
                harita.fitBounds(ref.getBounds(), { padding: [30, 30] });
            } else if (refKoordinat.lat && refKoordinat.lng) {
                harita.setView([refKoordinat.lat, refKoordinat.lng], 17);
            }
        } catch(e) {}
    }
    ...
    // ---------- Nokta ekle (çizim modu) ----------
    function noktaEkle(latlng) {
        cizim.noktalar.push(latlng);
        const idx = cizim.noktalar.length - 1;
        const marker = L.marker(latlng, {
            icon: L.divIcon({
                className: 'ec-vertex-wrap',
                html: idx === 0 ? '<div class="ec-vertex ec-vertex-first" title="İlk nokta"></div>' : '<div class="ec-vertex"></div>',
        ...
        cizim.tempMarkerlar.push(marker);
        onizleCiz();
    }
    ...
    function polygonKapat() {
        ...
        temizleTemp();
        cizim.noktalar = [];
        cizim.polygon = L.polygon(pts, {
            color: '#f43f5e', weight: 2, fillColor: '#f43f5e', fillOpacity: .28, interactive: true,
        }).addTo(harita);
        ...
        cizim.polygon.on('dblclick', function (e) {
            L.DomEvent.stopPropagation(e);
            if (confirm('Poligonu silmek istediğinize emin misiniz?')) polygonSil();
        });
        btnSil.hidden = false;
        formuGuncelle();
        modaGir('duzenle');
    ...
    function edgeGuncelle() {
        ...
        for (let i = 0; i < halka.length; i++) {
            ...
            m.on('dragstart', function () {
                halka.splice(idx, 0, m.getLatLng());
                cizim.polygon.setLatLngs([halka]);
            });
            m.on('drag', function (e) {
            ...
            m.on('dragend', function () { vertexCiz(); formuGuncelle(); });
    ...
    function formuGuncelle() {
        ...
        // Otomatik taşınmaz eşleme (debounced)
        tasinmazEsleDebounced(coords);
    }

    // ---------- Otomatik taşınmaz eşleme ----------
    const tsnInput = document.getElementById('tasinmaz_id');
    const tsnBilgi = document.getElementById('ec-tsn-bilgi');
    let esleTimer = null;

    function tasinmazEsleDebounced(coords) {
        clearTimeout(esleTimer);
        esleTimer = setTimeout(() => tasinmazEsle(coords), 450);
    }

    async function tasinmazEsle(coords) {
        if (!coords || coords.length < 3) return;
        // Poligonun kapanış noktasını at
        ...
        try {
            ...
            tsnBilgiGuncelle('bulunamadi', 'Eşleme sırasında hata oluştu.');
        }
    }

    function tsnBilgiGuncelle(tip, veri) {
        const html = {
            arama: `<div class="ec-tsn-durum is-arama"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="animation:ec-pulse 1s infinite;"><circle cx="12" cy="12" r="9"/></svg><div>${veri}</div></div>`,
            bulundu: `<div class="ec-tsn-durum is-bulundu"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12l5 5L20 7"/></svg><div><strong>Ada ${escapeHtml(veri.ada ?? '—')} / Parsel ${escapeHtml(veri.parsel ?? '—')}</strong><small>${escapeHtml(veri.il ?? '')} · ${escapeHtml(veri.ilce ?? '')} · ${escapeHtml(veri.mahalle ?? '')}</small></div></div>`,
            ...
            bos: `<div class="ec-tsn-durum is-bos"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2L2 8l10 6 10-6z"/><path d="M2 12l10 6 10-6"/></svg>Haritada bir alan çizin; içinde bir taşınmaz varsa otomatik bağlanacak.</div>`,
        }[tip];
    ...
    harita.on('click', function (e) {
        if (cizim.modu === 'cizim') noktaEkle(e.latlng);
    });
    harita.on('mousemove', function (e) {
        if (cizim.modu !== 'cizim' || cizim.noktalar.length === 0) return;
        cizim.farePos = e.latlng;
        onizleCiz();
    });
    harita.on('dblclick', function () { if (cizim.modu === 'cizim' && cizim.noktalar.length >= 3) polygonKapat(); });
    ...
    async function mevcutlariYukle() {
        try {
            let url = API_MEVCUT;
            if (HARIC_ID) url += '?haric=' + HARIC_ID;
            ...
            (fc.features || []).forEach(f => {
                ...
                layer.eachLayer(l => {
                    // Tooltip
                    l.bindTooltip(
                        `<strong>${escapeHtml(p.ad_soyad_unvan ?? '—')}</strong>
                        <div class="sub">Ada ${escapeHtml(p.ada ?? '—')} / Parsel ${escapeHtml(p.parsel ?? '—')} · ${escapeHtml(p.nitelik ?? '')}</div>
                        <span class="aksiyon">▸ Tıklayın: bu alana yeni ecrimisil ekleyin</span>`,
                        { className: 'ec-mvt-tooltip', sticky: true }
                    );
                    // Click: mevcut poligonu kopyala
                    l.on('click', e => {
                        L.DomEvent.stopPropagation(e);
                        if (!confirm(`"${p.ad_soyad_unvan}" işgalcisiyle aynı alana yeni bir ecrimisil kaydı eklemek istiyor musunuz?\n\nAynı poligon kullanılacak, taşınmaz ${p.ada}/${p.parsel} otomatik bağlanacak.`)) return;
                        mevcuttanKopyala(f);
                    });
                    // Hover — belirginleştir
                    l.on('mouseover', () => l.setStyle({ fillOpacity: .28, weight: 2.5 }));
                    l.on('mouseout',  () => l.setStyle({ fillOpacity: .10, weight: 1.8 }));
                });
            });
        } catch(e) { console.warn('Mevcut ecrimisiller yüklenemedi', e); }
    }

    function mevcuttanKopyala(feature) {
        // Mevcut poligonu bizim çizim katmanımıza al
        if (cizim.polygon) { harita.removeLayer(cizim.polygon); cizim.polygon = null; }
