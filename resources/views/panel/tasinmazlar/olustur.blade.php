        ...
        <div class="hrt-shell" id="hrt-shell">
            ...
            <div class="hrt-body">
                <div id="harita" class="hrt-map"
                     data-lat="{{ old('lat') }}"
                     data-lng="{{ old('lng') }}"
                     data-koordinat='{{ old('koordinat') }}'></div>
            </div>
        </div>
        ...
        <div class="form-row form-row-5">
            <div class="form-field {{ $errors->has('il_id') ? 'has-error' : '' }}">
                ...
                <select id="il_id" name="il_id" class="form-select" data-cascade="il" required>
                    <option value="">— İl seçin —</option>
                    @foreach ($iller as $il)
                        <option value="{{ $il->id }}" data-tkgm-id="{{ $il->tkgm_id }}" @selected(old('il_id') == $il->id)>{{ $il->ad }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-field {{ $errors->has('ilce_id') ? 'has-error' : '' }}">
                ...
                <select id="ilce_id" name="ilce_id" class="form-select" data-cascade="ilce" disabled required>
                    <option value="">— Önce il seçin —</option>
                </select>
            </div>
            ...
            <div class="form-field {{ $errors->has('mahalle_id') ? 'has-error' : '' }}">
                ...
                <select id="mahalle_id" name="mahalle_id" class="form-select" data-cascade="mahalle" disabled required>
                    <option value="">— Önce ilçe seçin —</option>
                </select>
            ...
            <div class="form-field {{ $errors->has('ada') ? 'has-error' : '' }}">
                ...
                <input id="ada" name="ada" type="text" class="form-input" value="{{ old('ada') }}" required maxlength="20" placeholder="123">
            ...
            <div class="form-field {{ $errors->has('parsel') ? 'has-error' : '' }}">
                ...
                <input id="parsel" name="parsel" type="text" class="form-input" value="{{ old('parsel') }}" required maxlength="20" placeholder="45">
                ...
                @if ($mudurlukSecilebilir ?? false)
                    <select id="mudurluk_id" name="mudurluk_id" class="form-select">
                        ...
                        @foreach ($mudurlukler ?? [] as $m)
                            <option value="{{ $m->id }}" @selected(old('mudurluk_id') == $m->id)>{{ $m->ad }}</option>
                        @endforeach
                    </select>
                    <span class="hint">Kayıt hangi müdürlüğe ait olacak.</span>
                @else
                    <input type="text" class="form-input" value="{{ auth()->user()->mudurluk->ad ?? 'Müdürlük atanmamış' }}" disabled>
                    <span class="hint">Kayıt sizin müdürlüğünüze yazılır.</span>
                @endif
    ...
    <section class="form-section">
        ...
        <div class="form-row form-row-2">
            <div class="form-field {{ $errors->has('alan') ? 'has-error' : '' }}">
                ...
                @php
                    ...
                    if ($alanEski !== null && $alanEski !== '') {
                        // TR biçimini normalize et sonra TR'ye çevir: "1234.56" veya "1.234,56" → 1234.56 → "1.234,56"
                        $tmp = str_contains($alanEski, ',') && str_contains($alanEski, '.')
                            ? str_replace(',', '.', str_replace('.', '', $alanEski))
                            : str_replace(',', '.', $alanEski);
            ...
            <div class="form-field {{ $errors->has('nitelik') ? 'has-error' : '' }}">
                ...
                <input id="nitelik" name="nitelik" type="text" class="form-input" value="{{ old('nitelik') }}" required maxlength="100" placeholder="Arsa · Tarla · Bahçe · Bağ">
                ...
                <div class="mks-alan">
                    <select id="imar_durumu_id" name="imar_durumu_id" class="form-select" required>
                        ...
                        @foreach ($imarDurumlari as $id)
                            <option value="{{ $id->id }}" @selected(old('imar_durumu_id') == $id->id)>{{ $id->ad }}</option>
                        @endforeach
                    </select>
        ...
        <div class="form-row form-row-2">
            <div class="form-field {{ $errors->has('emsal') ? 'has-error' : '' }}">
                ...
                @php
                    ...
                    if ($emsalEski !== null && $emsalEski !== '') {
                        $tmp = str_contains($emsalEski, ',') && str_contains($emsalEski, '.')
                            ? str_replace(',', '.', str_replace('.', '', $emsalEski))
                            : str_replace(',', '.', $emsalEski);
            ...
            <div class="form-field {{ $errors->has('yenaz_yencok') ? 'has-error' : '' }}">
                ...
                <input id="yenaz_yencok" name="yenaz_yencok" type="text" class="form-input" required
                       value="{{ old('yenaz_yencok') }}" maxlength="50"
        ...
        <div class="form-row">
            <div class="form-field {{ $errors->has('imar_notu') ? 'has-error' : '' }}">
                ...
                <textarea id="imar_notu" name="imar_notu" class="form-textarea" rows="3" maxlength="5000"
                          placeholder="Plan notları, lejant açıklamaları...">{{ old('imar_notu') }}</textarea>
