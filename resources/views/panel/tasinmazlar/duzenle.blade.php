...
<form method="POST" action="{{ route('panel.tasinmazlar.guncelle', $tasinmaz->id) }}" class="form-shell" enctype="multipart/form-data" novalidate>
    @csrf
    @method('PUT')
                ...
                <select id="muhasebe_kayit_id" name="muhasebe_kayit_id" class="form-select" data-tree="1" required>
                    ...
                    @foreach ($muhasebeKayitlari as $mk)
                        <option value="{{ $mk['id'] }}"
                                ...
                                @selected(old('muhasebe_kayit_id', $tasinmaz->muhasebe_kayit_id) == $mk['id'])>{{ $mk['etiket'] }}</option>
                    @endforeach
                </select>
                ...
                <select id="kayit_turu_id" name="kayit_turu_id" class="form-select" data-tree="1" required>
                    ...
                    @foreach ($kayitTurleri as $kt)
                        <option value="{{ $kt['id'] }}"
                                ...
                                @selected(old('kayit_turu_id', $tasinmaz->kayit_turu_id) == $kt['id'])>{{ $kt['etiket'] }}</option>
                    @endforeach
                </select>
        ...
        <div class="form-row form-row-2">
                ...
                <div class="mks-alan">
                    <select id="mevcut_kullanim_sekli" name="mevcut_kullanim_sekli" class="form-select" required>
                        ...
                        @foreach ($mevcutKullanimSekilleri as $mks)
                            <option value="{{ $mks->ad }}" @selected(old('mevcut_kullanim_sekli', $tasinmaz->mevcut_kullanim_sekli) === $mks->ad)>{{ $mks->ad }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-field">
                ...
                <select id="isgal_durumu" name="isgal_durumu" class="form-select" required>
                    <option value="">— Seçiniz —</option>
                    <option value="yok" @selected(old('isgal_durumu', $tasinmaz->isgal_durumu) === 'yok')>Yok</option>
                    <option value="var" @selected(old('isgal_durumu', $tasinmaz->isgal_durumu) === 'var')>Var</option>
                    <option value="kismi" @selected(old('isgal_durumu', $tasinmaz->isgal_durumu) === 'kismi')>Kısmi</option>
                </select>
        ...
        <div class="form-row form-row-2">
            ...
            <div class="form-field">
                ...
                <label class="form-check">
                    ...
                    <input type="checkbox" name="meclis_satis_karari_var" value="1" @checked(old('meclis_satis_karari_var', $tasinmaz->meclis_satis_karari_var))>
    ...
    <section class="form-section">
        ...
        @php
            $bbnMevcut = $tasinmaz->bagimsizBolumler->first();
            $bbn = old('bagimsiz_bolum', $bbnMevcut ? $bbnMevcut->only([
                'blok_no', 'kat_no', 'bagimsiz_bolum_no', 'nitelik',
                'brut_alan', 'net_alan', 'oda_sayisi', 'cephe',
                'mevcut_kullanim_sekli', 'isgal_durumu',
            ]) : []);
    ...
    <section class="form-section">
        ...
        <div class="form-row">
            <div class="form-field {{ $errors->has('aciklama') ? 'has-error' : '' }}">
                ...
                <textarea id="aciklama" name="aciklama" class="form-textarea" rows="4" maxlength="5000" placeholder="Ek notlar, gözlemler veya önemli bilgiler...">{{ old('aciklama', $tasinmaz->aciklama) }}</textarea>
    ...
    <section class="form-section">
        ...
        @php
            $hisselerEski = old('hisseler', $tasinmaz->hisseler->map(fn($h) => [
                'hisse_no' => $h->hisse_no,
                'hisse_yuzolcum' => $h->hisse_yuzolcum,
                'edinme_sekli' => $h->edinme_sekli,
                'yevmiye_no' => $h->yevmiye_no,
                'edinme_tarihi' => optional($h->edinme_tarihi)->format('Y-m-d'),
                'kayitlardan_cikis' => $h->kayitlardan_cikis,
                'kayitlardan_cikistarihi' => optional($h->kayitlardan_cikistarihi)->format('Y-m-d'),
                'hisse_durum' => $h->hisse_durum,
                'islem_tipi' => $h->islem_tipi,
                'maliyet_bedeli' => $h->maliyet_bedeli,
                'rayic_bedel' => $h->rayic_bedel,
                'emlak_vd' => $h->emlak_vd,
                'iz_bedeli' => $h->iz_bedeli,
            ])->toArray());
        @endphp

        <div class="hisse-shell" id="hisse-shell" data-eski='@json($hisselerEski)'>
